...
| Approval | `approval_id` | 高风险审批资源，生产 v1 目标 |
| ModelCandidate | `model_id` | 通过门禁的候选模型，生产 v1 目标 |
| WebhookEndpoint | `webhook_endpoint_id` | 事件订阅端点，生产 v1 目标 |
| WebhookDelivery | `delivery_id` | Webhook 投递记录，生产 v1 目标 |
| DeletionJob | `deletion_id` | 删除 saga 跟踪资源，生产 v1 目标 |
...
| `GET` | `/v1/runs/{id}/approvals` | `listRunApprovals` | 空页占位 | 返回审批对象 |
| `POST` | `/v1/runs/{id}/approvals/{approval_id}:decide` | `decideApproval` | `501` | 提交审批决定 |
...
| `GET` | `/v1/models/{id}` | `getModelCandidate` | `404` | 返回已注册候选模型 |
| `POST/GET/DELETE` | `/v1/webhook-endpoints...` | 多个 Webhook operation | `501` | 完整 Webhook 管理 |
| `DELETE` | `/v1/datasets/{id}` | `deleteDataset` | `501` | 启动删除 saga |
| `GET` | `/v1/deletions/{id}` | `getDeletionJob` | `404` | 查询删除任务 |
...
Webhook 是生产 v1 目标能力，当前 0.7.0 路由返回 `501`。正式设计：
...
当前成功 Run 返回 `NO_ELIGIBLE_MODEL`。生产 v1 中，只有满足以下条件才返回 `ELIGIBLE_MODEL_AVAILABLE`：
